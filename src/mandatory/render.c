...
/*   Updated: 2024/03/13 20:11:10 by bedos-sa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_square(t_cub3d *cub3d, t_vector map_point, size_t size, uint32_t color)
{
	for(size_t y = map_point.y; y <= (map_point.y + size); y++)
	{
		...
		{
			if (y == (map_point.y + size) || x == (map_point.x + size) || \
					y == map_point.y || x == map_point.x)
...
void	render_player(t_cub3d *cub3d)
{
	...
	// we make a shift to the left & upper sides so to render around our player's coordinates, 
	// that way, the player's x and y will be at the center of our square, not in the top left
